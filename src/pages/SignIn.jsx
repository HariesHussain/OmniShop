...
export default function SignIn() {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            ...
            setMsg("Firebase password reset link sent to your email!");
        } catch {
    ...
    const handleSubmit = async (e) => {
        ...
        const safePass = password.slice(0, 128);
        ...
        setLoading(true);
        try {
            await login(safeEmail, safePass);
            navigate("/");
        } catch {
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-card border border-border rounded-2xl p-8 shadow-sm">
                    <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
                        ...
                        {msg && (
                            <p className="text-emerald-500 text-sm bg-emerald-50 rounded-lg px-3 py-2">{msg}</p>
                        )}
                        <Button
                            type="submit"
                            disabled={loading}
                    ...
                    <div className="mt-4 flex justify-between items-center text-sm">
                        <button onClick={handleResetPassword} type="button" className="text-orange-600 hover:underline">Forgot Password?</button>
                    </div>
                    ...
                    <p className="text-center text-sm text-muted-foreground mt-4">
                        New to OmniShop? <Link to="/register" className="text-orange-500 hover:underline font-medium">Create account</Link>
                    </p>
                </div>
            </div>
        </div>
